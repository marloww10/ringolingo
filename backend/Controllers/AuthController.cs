...
namespace Ringolingo.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [HttpGet("loginSocial")]
        public async Task<ActionResult> LoginSocial([FromQuery] string? nome)
        {
            ...
            var nomeFinal = nome ?? User.FindFirst("full_name")?.Value ?? User.FindFirst("name")?.Value;

            if (string.IsNullOrEmpty(email) || string.IsNullOrEmpty(supabaseId))
                return Unauthorized("Token inválido ou incompleto.");

            var resposta = await _authService.LoginSocial(email, supabaseId, nomeFinal);
            return Ok(resposta);
        }
    }
}